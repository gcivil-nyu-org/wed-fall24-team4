...
class ReportingTests(TestCase):
    def setUp(self):
        ...
        self.user_list = [ User.objects.create_user(
    ...
    def test_auto_broadcast(self):
        ...
        for x in self.user_list[:3]:
            ...
            response = self.client.post(
                url, {"station": "191 ST", "infrastructure": "elevator", "status": "broken"}
            )
            self.client.logout()
        ...
        for x in self.user_list[6:8]:
            self.client.force_login(x)
            response = self.client.post(
                url, {"station": "191 ST", "infrastructure": "elevator", "status": "maintenance"}
            )
            self.client.logout()
